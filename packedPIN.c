/*
        a) How might we use the existing 32bit storage scheme in place for the
   6 numeric digit to store the alpha-numeric PIN?
        Since there are 30 symbols to be represented, and we have only 6 symbols
   to be packed in 32 bits, each symbol should be represented in 5 bits. I am
   choosing the simplest notation of A=1, C=2,.... and 2=24, 3=25, 9=30 as the
   encoding scheme.
         
        b) Write a	piece of code to encode	OR	decode your
   encoding scheme:
*/
